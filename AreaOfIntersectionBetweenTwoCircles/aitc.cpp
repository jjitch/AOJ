...
// ‰~‚ğ‰~üã‚Ì2“_‚ğŒ‹‚Ôü•ª‚Å•ªŠ„‚µAü•ª‚Ì¶è‘¤‚Ì–ÊÏ‚ğ•Ô‚·B
...
double intersectionArea(const Circle& cirA, const Circle& cirB)
{
	double res = 0;
	const double shorterR = std::min(cirA.getRadius(), cirB.getRadius());
	...
	{
		std::vector<Vec2> intersectionPoint = intersectionPoints(cirA, cirB);
		Segment splitSegment{ intersectionPoint[0], intersectionPoint[1] };
		res = splitCircle(cirB, splitSegment) - splitCircle(cirA, splitSegment) + cirA.getRadius() * cirA.getRadius() * M_PI;
	}
	break;
	case PAIR_OF_CIRCLES::INSCRIBED:
		res = shorterR * shorterR * M_PI;
		break;
